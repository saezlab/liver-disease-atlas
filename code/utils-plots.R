...
my_theme = function(grid = NULL, fsize = 11) {
  ...
  if (is.null(grid)) {
    p = cowplot::background_grid(major = "xy", minor = "none", size.major = 0.4)
  } else if (grid == "y") {
    p = cowplot::background_grid(major = "y", minor = "none", size.major = 0.4)
  } else if (grid == "x") {
    p = cowplot::background_grid(major = "x", minor = "none", size.major = 0.4)
  } else if (grid == "no") {
    p = theme()
  }
  p = p +
    theme(title = element_text(size=fsize + 1),
    ...
    plots = c("up", "down") %>%
      map(function(r) {
        ...
        draw.pairwise.venn(
          ...
          as_ggplot() %>%
          grid.arrange(top = textGrob(r, gp=gpar(fontsize=15,
                                                 fontface = "bold")))
      })
...
plot_stem_profiles = function(df, p_cutoff = 0.05, model_profile = F,
  ...
  if (n_distinct(df$key) != 1) {
    stop("Only one 'key' must be present. Currenty the keys are: ",
  ...
  profile_summary = df %>%
    ...
    dplyr::summarize(m = median(value),
              s = sd(value),
              lower = m - s,
              upper = m + s) %>%
    ungroup()
