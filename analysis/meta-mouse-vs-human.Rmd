...
combined_meta <- bind_rows(
  ...
  mutate(class = case_when(str_detect(
    group, "early|control|stage_0|mild") ~ "control",
    TRUE ~ "disease"))
...
chronic <- readRDS(here("output/mouse-chronic-ccl4/limma_result_hs.rds")) %>%
  filter(contrast_reference == "pure_ccl4") %>%
  select(gene, contrast, logfc = logFC, fdr, regulation, 
         source = contrast_reference) %>%
  assign_deg(fdr_cutoff = 0.05, effect_size_cutoff = log2(1.5), 
...
pr = setup %>%
  mutate(metrics = pmap(., .f = function(mm_genes, hs_genes, ...) {
    ...
    tibble(recall = nrow(z)/nrow(hs_genes),
