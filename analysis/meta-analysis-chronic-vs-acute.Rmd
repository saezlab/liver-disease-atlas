...
This analysis computes the similarity of differential expressed genes for specific contrast of the acute mouse models. Similarity is measured by the Jaccard Index.
```{r deg-similarity}
...
gsea_res_top %>%
  ...
  facet_rep_wrap(~direction, scales = "free") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
Filter for differential expressed genes of the acute mouse model. Visual inspection suggest that the differential expressed genes are consistently regulated across the acute mouse models.
```{r acute-pool}
contrasts <- readRDS(here(output_path, "limma_result.rds")) %>%
...
Heatmap(t(mat_exclusive_chronic_genes),
  ...
  row_names_gp = gpar(fontsize = fz), column_names_gp = gpar(fontsize = fz),
  name = "logFC",
  row_gap = unit(2.5, "mm"),
  border = T,
